 		...
 		\subsubsection{Beispiel}
 			...
 			\begin{algorithm}
 				...
 				\Fn {\FPower{a : $ \mathbb{ R },  n_0 : \mathbb{ N }   $   } : $ \mathbb{ R } $ }{ 		\tcp*{\(a^{n_0}\) }
 					...
 					\BlankLine
 					\While{ \( n > 0 \)}{
 						\textcolor{purple}{\Assert \( p^n r = a ^{n_0} \) 		\tcp*{Schleifeninvariante} }
 						\BlankLine
 						\If{n is odd}{
 							\(n--\) \;
 							\( r \gets r \times p  \) \;
...
\section[Mütter und Väter aller Datenstrukturen]{Folgen, Felder, Listen}
	...
	\subsection{Verkettete Listen}
		...
		\paragraph{Procedure splice()}
			...
			Nach Splice liegen die Elemente nicht mehr in der gleichen Reihenfolge im Speicher wie durch die Liste repräsentiert\\
	...
	\subsection{Arrays}
		...
		\paragraph{Teilweise ungenutztem Speicher}
			...
			-Beim nächsten Mal push muss das Array vergrößert werden, schlechte Laufzeit und keine Amortation.\\
...
\section[Effizienz durch Ordnung]{Sortieren}
	...
	\begin{algorithm}
		...
		\Function{mergeSort(\( \langle e_1 \dots e_n \rangle \)) : Sequence of Element}{
			...
			\Return{ merge( mergeSort(\( e_1 \dots e_{\lfloor \frac{n}{2} \rfloor } \rangle \)), mergeSort(\(e_{\lfloor \frac{n}{2} \rfloor + 1} \dots e_n \rangle \)  ) }
		}
		}
	\end{algorithm}
